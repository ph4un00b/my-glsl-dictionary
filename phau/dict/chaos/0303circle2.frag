...
void main(){
    ...
    if (mod(floor(u_time), 3.0) == 0.0) {
        ...
        color += distance(
            vec2(
                // fract( sin (dot(st.x, 12.) ) * K ),
                st.x,
                // fract( sin (dot(st.y, 78.) ) * K ) 
                st.y
            )
            ,vec2(0.5));
        color += step(0.400, color);
        
    } else if (mod(floor(u_time), 3.0) == 1.0) {
        ...
        // angle = rand(TWO_PI); /** 2 */
        float r1 = rand(1.);
        // r1 = 1. - (rand( rand(1.000) ));
        // r1 = 1. - rand(rand( rand(1.000) ));
        // r1 = 1. - rand(rand(rand( rand(1.000) )));
        // r1 = 1. - sqrt(rand(1.));
        float radius = 0.136;
        // pos.x = pos.x + r + cos(angle) * radius;
        // pos.y = pos.y + r + sin(angle) * radius;
        ...
        // enclose noise in circle
        color += distance(
            pos
            ,vec2(0.000,0.020));
        // color =+ circle(pos, -0.796);
        color += step(0.4, color);
        // color = 0.256 - color;
    } else if (mod(floor(u_time), 3.0) == 2.0) {
        float angle = atan(
            (pos.x),
            (pos.y)
        );
