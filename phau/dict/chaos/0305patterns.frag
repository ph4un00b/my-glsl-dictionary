...
float pattern(in vec2 _st, in float _index, in float size){
    _index = fract(((_index-0.5)*2.0));
    float color = 0.0;
    if (_index > 0.75) {
        color += lineL2R(_st, size);
    } else if (_index > 0.5) {
        color += lineR2L(_st, size); 
    } else if (_index > 0.25) {
        color += lineL2R(_st, size);
    } else if (_index > 0.0) {
        color += lineR2L(_st, size); 
...
void main(){
    ...
    // color += lineL2R(tile, ancho); 
    color += lineR2L(tile, ancho); 
    // color += step(tile.x, tile.y);
    color += distance(pos,vec2(0.000,0.00));
    color = step(0.4, color);
    // remmaping [0, 1]
	float idx = fract((( random( ipos ) - 0.5) * 2.0));
	vec3 rgb = vec3(color);
	// debug
	if (idx > 0.750) {
	rgb += vec3(0.061,0.095,0.940);
	} else if (idx > 0.5) {
	rgb += vec3(0.237,0.940,0.121);
	} else if (idx > 0.25) {
	rgb += vec3(0.940,0.137,0.209);
	} else if (idx >= 0.0) {
	rgb += vec3(0.940,0.056,0.866);
	}
